...
void __kmpc_end_serialized_parallel(ident_t *loc, kmp_int32 global_tid) {
  ...
  if (ompt_enabled.enabled &&
    ...
    if (ompt_enabled.ompt_callback_parallel_end) {
      // This is executed in one of the following cases:
      // 1. gcc is used to generate the code of serialized parallel region
      //    and this function is called from within the internal implementation
      //    of the runtime (__kmp_join_call inside kmp_runtime.cpp).
      // 2. clang is used to generate the code of serialized parallel region
      //    and omp parallel directive contains the clause if(0).
      //    This function is invoked directly from the application code
      //    and the call is generated by the clang compiler.
      // For the first case, ompt_parallel_invoker_runtime should be passed
      // to the dispatched callback, while for the second case,
      // ompt_parallel_invoker_program should be passed.
      // Invoking this function from within the runtime is inherently wrong,
      // because it introduces unnecessary complexity. Namely, to distinct
      // one of the above cases, it is required to be able to dynamically
      // determine the caller of the function.
      // To do this, check whether return_address exists. Note that it doesn't
      // exist if the clang is used, since the return address was set inside
      // __kmpc_serialized_parallel that has already been finished so the
      // corresponding guard is destroyed and the address is cleared.

      // Determine whether the clang is used and if the function is invoked
      // directly by the application (second case).
      bool clang_if0 = this_thr->th.ompt_thread_info.return_address == NULL;
      int invoker = clang_if0 ? ompt_parallel_invoker_program
                              : ompt_parallel_invoker_runtime;
      // This is obvious a hack. It would be smarter to refactor the runtime
      // to avoid invoking __kmpc_* functions from within the runtime itself.
      // The return_address maintenance should be fixed inside
      // __kmpc_serialized_parallel too.
