...
// If TRACE_ALL is defined, all events are traced, whether an ERROR is detected
// or not. If NO_TIMESTAMPS is defined, the trace will not have timestamps;
// otherwise it will. If NO_CALLBACKS is defined, no callbacks will be set, so
// none will appear in the trace. If NO_NONEST is defined, do not run the
// testtriple_nonest subtest
...
void skew_delay(int);
void format_sync_type(ompt_sync_region_t, ompt_scope_endpoint_t endpoint,
                      char *);
...
void ompt_implicit_task(ompt_scope_endpoint_t, ompt_data_t *, ompt_data_t *,
                        unsigned int, unsigned int, int);
void ompt_thread_begin(ompt_thread_t, ompt_data_t *);
void ompt_thread_end(ompt_data_t *);
void ompt_parallel_begin(ompt_data_t *, const ompt_frame_t *, ompt_data_t *,
                         unsigned int, int, const void *);
void ompt_parallel_end(ompt_data_t *, ompt_data_t *, int, const void *);
void ompt_task_create(ompt_data_t *, const ompt_frame_t *, ompt_data_t *, int,
                      int, const void *);
void ompt_task_schedule(ompt_data_t *, ompt_task_status_t, ompt_data_t *);
void ompt_implicit_task(ompt_scope_endpoint_t, ompt_data_t *, ompt_data_t *,
                        unsigned int, unsigned int, int);

void ompt_targetcb(ompt_target_t, ompt_scope_endpoint_t, int, ompt_id_t,
                   ompt_id_t, const void *);

void ompt_target_data_op(ompt_id_t, ompt_id_t, ompt_target_data_op_t, void *,
                         int, void *, int, size_t, const void *);
void ompt_target_submit(ompt_id_t, ompt_id_t, unsigned int);

void ompt_control_tool(uint64_t, uint64_t, void *, const void *);
void ompt_device_initialize(int, const char *, ompt_device_t *,
                            ompt_function_lookup_t, const char *);
void ompt_device_finalize(int);
void ompt_device_load(int, const char *, int64_t, void *, size_t, void *,
                      void *, uint64_t);
void ompt_device_unload(int, uint64_t);

void ompt_sync_region_wait(ompt_sync_region_t, ompt_scope_endpoint_t,
...
void ompt_work(ompt_work_t, ompt_scope_endpoint_t, ompt_data_t *, ompt_data_t *,
               uint64_t, const void *);
void ompt_master(ompt_scope_endpoint_t, ompt_data_t *, ompt_data_t *,
                 const void *);
void ompt_target_map(ompt_id_t, unsigned int, void **, void **, size_t *,
                     unsigned int *, const void *);
void ompt_sync_region(ompt_sync_region_t, ompt_scope_endpoint_t, ompt_data_t *,
                      ompt_data_t *, const void *);
void ompt_lock_init(ompt_mutex_t, unsigned int, unsigned int, ompt_wait_id_t,
                    const void *);
void ompt_lock_destroy(ompt_mutex_t, ompt_wait_id_t, const void *);
void ompt_mutex_acquire(ompt_mutex_t, unsigned int, unsigned int,
...
void ompt_reduction();
void ompt_dispatch(ompt_data_t *, ompt_data_t *, ompt_dispatch_t,
