...
#pragma omp for schedule(static)
    for (i = 0; i < N; i++)
      (*validate_ptr)("for schedule(static)");
#pragma omp master
    (*delay_ptr)(10);
...
#pragma omp for schedule(dynamic)
    for (i = 0; i < N; i++)
      (*validate_ptr)("for schedule(dynamic)");
#pragma omp master
    (*delay_ptr)(10);
...
#pragma omp parallel for schedule(static) private(i)
  for (i = 0; i < N; i++)
    (*validate_ptr)("parallel for static");
}
...
#pragma omp parallel for schedule(dynamic) private(i)
  for (i = 0; i < N; i++)
    (*validate_ptr)("parallel for dynamic");
}
...
#pragma omp parallel for schedule(guided) private(i)
  for (i = 0; i < N; i++)
    (*validate_ptr)("parallel for guided");
}
...
void form_label(char *buffer, char *label) {
