...
#pragma omp parallel private(i)
  {
    (*vi3_validate_ptr)("parallel begin", APP);


#pragma omp master
    ts_write("\n                  starting for\n\n");
#pragma omp for
    for (i = 0; i < N; i++) (*vi3_validate_ptr)("for", APP);
...
#pragma omp for schedule(static)
    for (i = 0; i < N; i++) (*vi3_validate_ptr)("for schedule(static)", APP);
...
#pragma omp for schedule(dynamic)
    for (i = 0; i < N; i++) (*vi3_validate_ptr)("for schedule(dynamic)", APP);
...
#pragma omp for schedule(guided)
    for (i = 0; i < N; i++) (*vi3_validate_ptr)("for schedule(guided)", APP);

#pragma omp master
    (*delay_ptr)(10);

    (*vi3_validate_ptr)("parallel end", APP);
  }
}
...
#pragma omp parallel for schedule(static) private(i)
  for (i = 0; i < N; i++) (*vi3_validate_ptr)("parallel for static", APP);
}
...
#pragma omp parallel for schedule(dynamic) private(i)
  for (i = 0; i < N; i++) (*vi3_validate_ptr)("parallel for dynamic", APP);
}
...
#pragma omp parallel for schedule(guided) private(i)
  for (i = 0; i < N; i++) (*vi3_validate_ptr)("parallel for guided", APP);
}
