...
void (*vi3_validate_ptr)(const char *, char) = validate_task_frame_hierarchy;
...
#define INITIAL_TASK_FRAME_CHECK

#define RUNTIME_FRAME_FLAGS (ompt_frame_runtime | ompt_frame_framepointer)

#define FP_CMP(frame1, which1, op, frame2, which2) \
  (frame1->which1 ## _frame.ptr op frame2->which2 ## _frame.ptr)


// Iterates over task frame hierarchy and checks whether the corresponding
// frames are initialized properly.
void validate_task_frame_hierarchy(const char *type, char call_site) {
  int thread_num;
  ompt_frame_t *task_frame;
  ompt_frame_t *child_task_frame;
  int task_type, ret, ancestor_level;
  ompt_data_t *task_data;
  ompt_data_t *parallel_data;
  char buf[256];
  ...
  for (ancestor_level = 0;; ancestor_level++) {
    ...
    if (ret == 0) {
      ...
      // No info about the task at this level. Continue further iterating over
      // ancestor hierarchy.
      continue;
    }
    // no other values of ret are allowed
    assert(ret == 2);
    assert(task_frame);

    if (ancestor_level == 0 && !(task_type & ompt_task_initial)) {
      if (call_site == APP) {
        // Thread is executing the application code, then

        // exit frame should be set
        if (task_frame->exit_frame.ptr == NULL) {
          assert(0);
        }
        // check exit_frame_flags
        if (task_frame->exit_frame_flags != RUNTIME_FRAME_FLAGS) {
          assert(0);
        }

        // enter_frame should be NULL
        if (task_frame->enter_frame.ptr != NULL) {
          assert(0);
        }

      } else {
        // Thread is executing the runtime code. Note that the task isn't
        // initial, so both exit and enter frame should be set.
        // Flags should be also set
        ...
        // exit_frame >= enter_frame
        if (!FP_CMP(task_frame, exit, >=, task_frame, enter)) {
          assert(0);
        }

      }

    } else if (task_type & ompt_task_initial) {
      // exit_frame should be NULL
      if (task_frame->exit_frame.ptr != NULL) {
        assert(0);
      }
      // enter_frame should be set
      if (task_frame->enter_frame.ptr == NULL) {
        assert(0);
      }
      ...
      // task_frame->exit_frame >= task_frame->enter_frame
      if (!FP_CMP(task_frame, exit, >=, task_frame, enter)) {
        assert(0);
      }

      // task_frame->enter_frame >= child_task_frame->exit_frame
      if (!FP_CMP(task_frame, enter, >=, child_task_frame, exit)) {
        assert(0);
      }
    }
