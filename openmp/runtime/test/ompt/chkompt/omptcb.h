...
ompt_get_task_info_t
    ompt_get_task_info_fn; // Address of routine to get task information
...
// ------------------------------------------------------------------------
char *cb_names[] = {"illegal callback number", //=0
...
#define SetCallback(type, name)                                                \
  ...
  if ((ret == ompt_set_error) || (ret == ompt_set_never)) {                    \
    fprintf(stderr,                                                            \
            "    Note: %s (%2d) is never triggered in this implementation of " \
            "OMPT (%d)\n",                                                     \
            cb_names[type], (int)type, (int)ret);                              \
  } else if (ret == ompt_set_impossible) {                                     \
    fprintf(stderr,                                                            \
            "    Note: %s (%2d) is impossible in this implementation of OMPT " \
            "(%d)\n",                                                          \
            cb_names[type], (int)type, (int)ret);                              \
  } else if ((ret == ompt_set_sometimes) ||                                    \
             (ret == ompt_set_sometimes_paired)) {                             \
    fprintf(stderr,                                                            \
            "    Note: %s (%2d) may or may not be triggered in this "          \
            "implementation of OMPT (%d)\n",                                   \
            cb_names[type], (int)type, (int)ret);                              \
  } else {                                                                     \
    ncallbacks++;                                                              \
  }
...
// ------------------------------------------------------------------------
void ompt_implicit_task(ompt_scope_endpoint_t endpoint,
...
// ------------------------------------------------------------------------
void ompt_thread_begin(ompt_thread_t thread_type, ompt_data_t *thread_data) {
  char *ctype = "unknown";
  switch (thread_type) {
...
// ------------------------------------------------------------------------
void ompt_thread_end(ompt_data_t *thread_data) {
...
// ------------------------------------------------------------------------
void ompt_parallel_begin(ompt_data_t *encountering_task_data,
...
// ------------------------------------------------------------------------
void ompt_parallel_end(ompt_data_t *parallel_data,
...
// ------------------------------------------------------------------------
void ompt_task_create(ompt_data_t *encountering_task_data,
                      const ompt_frame_t *encountering_task_frame,
...
void ompt_task_schedule(ompt_data_t *prior_task_data,
                        ompt_task_status_t prior_task_status,
                        ompt_data_t *new_task_data) {
  ck_ra("task_schedule_CB", 1, (const void *)1, 0,
...
// ------------------------------------------------------------------------
void ompt_targetcb(ompt_target_t kind, ompt_scope_endpoint_t endpoint,
                   int device_num, ompt_id_t task_data, ompt_id_t target_id,
                   const void *codeptr_ra) {
...
// ------------------------------------------------------------------------
void ompt_target_data_op(ompt_id_t target_id, ompt_id_t host_op_id,
...
// ------------------------------------------------------------------------
void ompt_target_submit(ompt_id_t target_id, ompt_id_t host_op_id,
...
// ------------------------------------------------------------------------
void ompt_work(ompt_work_t wstype, ompt_scope_endpoint_t endpoint,
               ompt_data_t *parallel_data, ompt_data_t *task_data,
...
// ------------------------------------------------------------------------
void ompt_master(ompt_scope_endpoint_t endpoint, ompt_data_t *parallel_data,
...
// ------------------------------------------------------------------------
void ompt_target_map(ompt_id_t id, unsigned int nitems, void **host_adder,
...
// ------------------------------------------------------------------------
void ompt_sync_region_wait(ompt_sync_region_t kind,
...
// ------------------------------------------------------------------------
void ompt_sync_region(ompt_sync_region_t kind, ompt_scope_endpoint_t endpoint,
                      ompt_data_t *parallel_data, ompt_data_t *task_data,
                      const void *codeptr_ra) {
  char buf[128];
  format_sync_type(kind, endpoint, buf);
...
// ------------------------------------------------------------------------
void ompt_lock_init(ompt_mutex_t kind, unsigned int hint, unsigned int impl,
...
// ------------------------------------------------------------------------
void ompt_lock_destroy(ompt_mutex_t kind, ompt_wait_id_t wait_id,
                       const void *codeptr_ra) {
  char buffer[128];
  format_lock_type(kind, buffer);
...
// ------------------------------------------------------------------------
void ompt_mutex_acquire(ompt_mutex_t kind, unsigned int hint, unsigned int impl,
                        ompt_wait_id_t wait_id, const void *codeptr_ra) {
  char buffer[128];
  format_lock_type(kind, buffer);
...
// ------------------------------------------------------------------------
void ompt_mutex_acquired(ompt_mutex_t kind, ompt_wait_id_t wait_id,
                         const void *codeptr_ra) {
  char buffer[128];
  format_lock_type(kind, buffer);
...
// ------------------------------------------------------------------------
void ompt_mutex_released(ompt_mutex_t kind, ompt_wait_id_t wait_id,
                         const void *codeptr_ra) {
  char buffer[128];
  format_lock_type(kind, buffer);
...
// ------------------------------------------------------------------------
void ompt_dependences(ompt_data_t *task_data, const ompt_dependence_t *deps,
                      int ndeps) {
...
// ------------------------------------------------------------------------
void ompt_task_dependence(ompt_data_t *src_task_data,
...
// ------------------------------------------------------------------------
void ompt_nest_lock(ompt_scope_endpoint_t endpoint, ompt_wait_id_t wait_id,
                    const void *codeptr_ra) {
...
// ------------------------------------------------------------------------
void ompt_flush(ompt_data_t *thread_data, const void *codeptr_ra) {
...
static char *ompt_cancel_flag_t_values[] = {
    "[parallel] ",  "[sections] ", "[loop] ",          "[taskgroup] ",
    "[activated] ", "[detected] ", "[discarded_task] "};
void ompt_cancel(ompt_data_t *task_data, int flags, const void *codeptr_ra) {
...
// ------------------------------------------------------------------------
void ompt_control_tool(uint64_t command, uint64_t modifier, void *arg,
                       const void *codeptr_ra) {
...
// ------------------------------------------------------------------------
void ompt_device_initialize(int device_num, const char *type,
...
void ompt_device_load(int device_num, const char *filename,
...
// ------------------------------------------------------------------------
void ompt_reduction(ompt_sync_region_t kind, ompt_scope_endpoint_t endpoint,
                    ompt_id_t parallel_id, ompt_id_t task_id,
                    const void *codeptr_ra) {
...
// OMPT callback for dispatch
// ------------------------------------------------------------------------
void ompt_dispatch(ompt_data_t *parallel_data, ompt_data_t *task_data,
...
// ------------------------------------------------------------------------
void format_sync_type(ompt_sync_region_t type, ompt_scope_endpoint_t endpoint,
                      char *buffer) {
  char *ctype = "unknown";
...
// ------------------------------------------------------------------------
void format_work_type(ompt_work_t type, ompt_scope_endpoint_t endpoint,
                      char *buffer) {
  char *ctype = "unknown";
  char *progress = buffer;
  if (endpoint == ompt_scope_begin) {
...
// ------------------------------------------------------------------------
void format_task_type(int type, char *buffer) {
...
// ------------------------------------------------------------------------
void format_lock_type(ompt_mutex_t type, char *buffer) {
  char *ctype = "unknown";
...
//
// ------------------------------------------------------------------------
void ck_ra(const char *type, int ckra, const void *ra, int param, char *desc) {
  int threadnum;
  char buf[512];
...
#pragma omp atomic update
    ...
    if (ckra == 0) {
      ...
      sprintf(buf, "%25s OK ck_ra  -- %sthread %3d, param = %d\n", type,
              (desc != NULL ? desc : ""), threadnum, param);
    } else if (ckra == 2) {
      sprintf(buf,
              "%25s OK ck_ra  -- %sthread %3d, param = %d codeptr_ra = %p -- "
              "in implicit task\n",
...
void validate(const char *type) {
  ...
  ompt_get_task_info_fn(0, // ancestor_level
                        NULL, // flags
                        NULL, // task_data
                        &task_frame,
                        NULL, // parallel_data
  ...
  // Check for failure
  if (task_frame == NULL) {
    sprintf(buf, "%25s -- ERROR  -- thread %3d task_frame = NULL\n", type,
            thread_num);
    ts_write(buf);
    error_breakpoint();
...
#pragma omp atomic update
    ...
    error_breakpoint();
    sprintf(buf, "%25s -- ERROR  -- thread %3d exit_frame.ptr = NULL\n", type,
            thread_num);
    ts_write(buf);
...
#pragma omp atomic update
    ...
    sprintf(buf, "%25s -- ERROR  -- thread %3d exit_frame.flags = 0\n", type,
            thread_num);
    ts_write(buf);
    error_breakpoint();
...
#pragma omp atomic update
    ...
    sprintf(buf, "%25s -- ERROR  -- thread %3d enter_frame.ptr != NULL\n", type,
            thread_num);
    ts_write(buf);
    error_breakpoint();
...
#pragma omp atomic update
    ...
    ompt_get_task_info_fn(1, // ancestor_level
                          NULL, // flags
                          NULL, // task_data
                          &parent_task_frame,
                          NULL, // parallel_data
    ...
    if (parent_task_frame == NULL) {
      sprintf(buf, "%25s -- ERROR  -- thread %3d parent_task_frame = NULL\n",
              type, thread_num);
      ts_write(buf);
      error_breakpoint();
...
#pragma omp atomic update
    ...
    sprintf(
        ...
        type, thread_num, task_frame->exit_frame.ptr,
        task_frame->exit_frame_flags);
    ts_write(buf);
#endif
  }
#ifdef RUN_SKEW
...
void ts_write(char *message) {
  ...
  sprintf(buf, "%4d.%09d: %s", sec, nsec, message);
  fwrite(buf, strlen(buf), 1, stderr);
#endif
}

hrtime_t gethrtime() { return ((hrtime_t)(omp_get_wtime() * 1.0E09)); }
